...
# -*- coding: utf-8 -*-
"""
MCP Orchestrator v3.7 (Context-Isolated + !command + caching + LLM Fallback + Auto-Index + Validation + Offline Search)
...
class TaskPlanner:
    ...
    def parse_intent(self, query: str) -> Dict:
        ...
        if any(kw in query_lower for kw in search_keywords):
            ...
            if is_online():
                ...
                # Офлайн: используем только локальные источники
                plan["steps"].append({
            ...
            if "из" in query_lower and "в" in query_lower:
                pass # перейдет к batch_move ниже
            else:
        ...
        # ─── Advanced FS Tools intents (mcp_fs_advanced) ─────────────────────
        elif "извлеки текст из папки" in query_lower or "прочитай все файлы в папке" in query_lower or "прочитай все тексты в папке" in query_lower:
            ...
            if not file_path:
                plan["intent"] = "error"
                plan["error"] = "Не удалось определить путь к файлу. Укажите полный путь, например: C:\\docs\\file.txt"
                plan["confidence"] = 0.0
                return plan
        ...
        elif any(kw in query_lower for kw in ["извлеки текст", "прочитай содержимое", "extract text", "что внутри", "текст из", "что написано в", "вытащи текст"]):
            file_path = self._extract_path(query)
            if not file_path:
                plan["intent"] = "error"
                plan["error"] = "Не удалось определить путь к файлу. Укажите полный путь."
                plan["confidence"] = 0.0
                return plan
            plan["intent"] = "extract"
            plan["params"]["file_path"] = file_path
        ...
        elif any(phrase in query_lower for phrase in ["найди в индексе", "поиск среди проиндексированных", "fts", "быстрый поиск", "найди в документах", "поиск по содержимому", "global search", "полнотекстовый"]):
            search_term = ""
            for kw in ["найди в индексе", "поиск среди проиндексированных", "fts", "быстрый поиск", "найди в документах", "поиск по содержимому", "global search", "полнотекстовый"]:
            ...
            if len(paths) >= 2:
                ...
                plan["params"]["source"] = src
                plan["params"]["target"] = dst
                plan["steps"].append({
            ...
            if path:
                ...
                plan["params"]["source"] = path
                plan["params"]["pattern"] = pattern
                plan["steps"].append({
    ...
    def execute_plan(self, plan: Dict, depth: int = 0) -> Dict:
        ...
        for i, step in enumerate(plan["steps"]):
            ...
            # Пропускаем сетевые инструменты, если нет подключения
...
def _llm_plan_query(query: str, dialog_id: str) -> Optional[Dict]:
    ...
    prompt = f"""You are a task planner. Given the user query, output a JSON plan with "intent", "steps" (each step has "tool" and "args"). Available tools: search_files, mempalace_search, rag_query, batch_move_files, sync_directories, excel_sort, run_shell, extract_text_from_file, extract_text_from_folder, version_file, index_all_files_content, search_all_indexed_files. Query: "{query}"."""
...
def run_task(natural_language_query: str) -> Dict:
    ...
    # ─── Обработка ошибок валидации (intent == "error") ──────────────────────
...
server = BaseMCPServer("orchestrator", "3.7")
