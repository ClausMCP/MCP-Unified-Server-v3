...
# -*- coding: utf-8 -*-
"""
MCP Goal Manager v1.0 – исправленная версия
Долгосрочное планирование: цели, подцели, зависимости, прогресс.
"""
...
from contextlib import contextmanager   # FIXED: добавлен импорт
...
class GoalsDB:
    ...
    @contextmanager   # FIXED: теперь contextmanager импортирован
    def _get_conn(self):
    ...
    def add_goal(self, goal_id: str, title: str, description: str,
                 ...
                 deadline: float = None, metadata: Dict = None) -> bool:
        now = time.time()
        with self._get_conn() as conn:
            try:
                conn.execute("""
                    ...
                    (goal_id, parent_id, title, description, goal_type, status, priority,
                     progress, created_at, updated_at, deadline, metadata)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    def update_goal(self, goal_id: str, **kwargs):
        allowed = ['title', 'description', 'status', 'priority', 'progress',
                   'updated_at', 'completed_at', 'deadline', 'metadata']
...
def check_condition(condition_type: str, condition_target: str) -> bool:
    ...
    try:
        if condition_type == "fact":
            ...
            with _graph_db._get_conn() as conn:
                row = conn.execute(
                    "SELECT 1 FROM entities WHERE name = ?", (condition_target,)
                ).fetchone()
                return row is not None
...
def goal_create(title: str, description: str, goal_type: str = "atomic",
    ...
    db = GoalsDB()
    ok = db.add_goal(goal_id, title, description, goal_type, parent_id, priority, deadline, metadata)
    if not ok:
...
server = BaseMCPServer("goal-manager", "1.0")
...
__mcp_plugin__ = {
    "name": "goal-manager",
    "version": "1.0",
    "description": "Управление целями, подцелями, зависимостями, интеграция с TaskManager и графом",
    "dependencies": [],
    "on_load": lambda: _log("[GoalManager] v1.0 loaded. Scheduler started."),
